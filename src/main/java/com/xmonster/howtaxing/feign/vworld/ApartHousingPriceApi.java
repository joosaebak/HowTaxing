...
@FeignClient(value = "apartHousingPrice", url="https://api.vworld.kr", configuration = {FeignConfiguration.class})
public interface ApartHousingPriceApi {
    @GetMapping("/ned/data/getApartHousingPriceAttr")
    ResponseEntity<String> getApartHousingPriceAttr(
            // 참고 : https://www.vworld.kr/dtna/dtna_apiSvcFc_s001.do?apiNum=31
            @RequestParam("pnu") String pnu,                    // 고유번호(8자리 이상)
            @RequestParam("stdYear") String stdYear,            // 기준연도(YYYY: 4자리)
            @RequestParam("dongNm") String dongNm,              // 동명
            @RequestParam("hoNm") String hoNm,                  // 호명
