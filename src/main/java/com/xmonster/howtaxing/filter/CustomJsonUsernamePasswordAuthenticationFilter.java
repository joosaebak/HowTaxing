package com.xmonster.howtaxing.filter;
...
import com.xmonster.howtaxing.service.user.LoginServiceImpl;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String DEFAULT_LOGIN_REQUEST_URL = "/user/socialLogin"; // "/login"으로 오는 요청을 처리
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
        ...
        System.out.println("[GGMANYAR]attemptAuthentication!!");
...
/*
        Map<String, String> socialLoginMap = objectMapper.readValue(messageBody, Map.class);

        SocialType socialType = SocialType.valueOf(socialLoginMap.get("userType"));
        String accessToken = socialLoginMap.get("accessToken");

        System.out.println("[GGMANYAR]socialType : " + socialType);
        System.out.println("[GGMANYAR]accessToken : " + accessToken);

        SocialLoginService loginService = this.getLoginService(socialType);
        SocialUserResponse socialUserResponse = loginService.getUserInfo(accessToken);

        UsernamePasswordAuthenticationToken authRequest = new UsernamePasswordAuthenticationToken(socialUserResponse.getEmail(), PasswordUtil.generateRandomPassword());//principal 과 credentials 전달
*/
